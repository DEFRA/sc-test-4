...
```bash
docker build --target development --no-cache --tag sc-test-4:development .
```

Run:

```bash
docker run -p 3000:3000 sc-test-4:development
```
...
```bash
docker build --no-cache --tag sc-test-4 .
```

Run:

```bash
docker run -p 3000:3000 sc-test-4
```
